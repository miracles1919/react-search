...
import { batch_deal_resume} from 'service/beiLib'
    ...
    btn_confirm = (event) =>{
      ...
      let emailPatten = new RegExp(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]+$/);
        for (var i = 0;i < _state.email.length;i++){
            ...
            if (_state.email[i] && (emailPatten.test(_state.email[i])!= true)){
                flag = true;
                break;
            }
      ...
      if(this.props.isforward){
        // console.log("不合适111",[this.props.list])
        batch_deal_resume({
          action:"forward",
          item_list:item_list
        }).then(res => {
        ...
        // console.log("转发评审222",[this.props.list])
        batch_deal_resume({
          action:"improper",
          item_list:item_list,
                    ...
                    <div className={style.content}>
                    {
                        ...
                        this.state.email.map(function(item,index){
                            ...
                            return (
                                ...
                                <div className={style.email_div} key={index}>
